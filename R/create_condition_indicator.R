...
species_condition_orig <- function(data, LWparams, species.codes) {
  ...
  # this isn't used in the calculations -- removing from new function
...
species_condition <- function(data, 
  ...
  condition <- cond.epu %>% 
    ...
    dplyr::rename(Time = YEAR,
                  Var = Species) %>% 
    dplyr::select(Var, EPU, Time, MeanCond, nCond) %>%
...
# dat <- species_condition_orig(survdat_subset)
# dat4 <- species_condition3(survdat_subset)
# 
# plot(dat$Time, dat$MeanCond)
# points(dat4$Time, dat4$MeanCond, col = "red")
