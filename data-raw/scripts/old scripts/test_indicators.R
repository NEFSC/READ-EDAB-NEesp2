...
bt_daily <- create_spatial_indicator(
...
pp <- create_spatial_indicator(
  indicator_name = "primary_production_mean",
...
sst_converted <- EDABUtilities::convert_longitude(
  data = here::here('data-raw/sst_2010.nc')
)

sst_weekly <- EDABUtilities::convert_longitude(
  data = here::here('data-raw/sst_weekly.nc')
)

sst_weekly <- create_spatial_indicator(
  indicator_name = 'sst',
  units = 'degC',
  data.in = sst_weekly,
  output.files = c(here::here('data-raw', 'sst.nc')),
...
sst_test <- EDABUtilities::convert_longitude(
  data = here::here('data-raw/icec.day.mean.2020.nc')
)
