...
smooth_hammerhead <- save_catch(this_species = "smooth hammerhead",
           out_folder = here::here("data-raw/hms_mrip"),
          catch_type = "landings")
...
scalloped_hammerhead <- save_catch(this_species = "scalloped hammerhead shark",
                                out_folder = here::here("data-raw/hms_mrip"),
                                catch_type = "landings")
...
great_hammerhead <- save_catch(this_species = "great hammerhead",
                                   out_folder = here::here("data-raw/hms_mrip"),
                                   catch_type = "landings")
gh_rds <- readRDS(here::here('data-raw/hms_mrip/catch_landings_great_hammerhead.Rds'))
gh_rds
gh_mrip <- create_total_rec_catch(gh_rds$data, remove_non_standard = FALSE)


atlantic_angel_shark <- save_catch(this_species = "atlantic angel shark",
                               out_folder = here::here("data-raw/hms_mrip"),
                               catch_type = "landings")
...
nurse_shark <- save_catch(this_species = "nurse shark",
                                   out_folder = here::here("data-raw/hms_mrip"),
                                   catch_type = "landings")
...
sand_tiger <- save_catch(this_species = "sand tiger",
                          out_folder = here::here("data-raw/hms_mrip"),
                          catch_type = "landings")
...
white_shark <- save_catch(this_species = "white shark",
                          out_folder = here::here("data-raw/hms_mrip"),
                          catch_type = "landings")
white_rds <- readRDS(white_shark)
white_rds
white_mrip <- create_total_rec_catch(white_rds$data, remove_non_standard = FALSE)


basking_shark <- save_catch(this_species = "basking shark",
                          out_folder = here::here("data-raw/hms_mrip"),
                          catch_type = "landings")
basking_rds <- readRDS(basking_shark)
basking_rds
basking_mrip <- create_total_rec_catch(basking_rds$data, remove_non_standard = FALSE)


porbeagle <- save_catch(this_species = "porbeagle",
                            out_folder = here::here("data-raw/hms_mrip"),
                            catch_type = "landings")
porbeagle_rds <- readRDS(porbeagle)
porbeagle_rds
porbeagle_mrip <- create_total_rec_catch(porbeagle_rds$data, remove_non_standard = FALSE)


thresher_shark <- save_catch(this_species = "thresher shark",
                        out_folder = here::here("data-raw/hms_mrip"),
                        catch_type = "landings")
thresher_rds <- readRDS(thresher_shark)
thresher_rds
thresher_mrip <- create_total_rec_catch(thresher_rds$data, remove_non_standard = FALSE)


bigeye <- save_catch(this_species = "bigeye thresher",
                        out_folder = here::here("data-raw/hms_mrip"),
                        catch_type = "landings")
bigeye_rds <- readRDS(bigeye)
bigeye_rds
bigeye_mrip <- create_total_rec_catch(bigeye_rds$data, remove_non_standard = FALSE)



mako <- save_catch(this_species = "shortfin mako",
                     out_folder = here::here("data-raw/hms_mrip"),
                     catch_type = "landings")
...
tiger_shark <- save_catch(this_species = "tiger shark",
                   out_folder = here::here("data-raw/hms_mrip"),
                   catch_type = "landings")
tiger_rds <- readRDS(tiger_shark)
tiger_rds
tiger_mrip <- create_total_rec_catch(tiger_rds$data, remove_non_standard = FALSE)


sharpnose <- save_catch(this_species = "atlantic sharpnose shark",
                   out_folder = here::here("data-raw/hms_mrip"),
                   catch_type = "landings")
sharpnose_rds <- readRDS(sharpnose)
sharpnose_rds
sharpnose_mrip <- create_total_rec_catch(sharpnose_rds$data, remove_non_standard = FALSE)


dusky_shark <- save_catch(this_species = "dusky shark",
                   out_folder = here::here("data-raw/hms_mrip"),
                   catch_type = "landings")
dusky_rds <- readRDS(dusky_shark)
dusky_rds
dusky_mrip <- create_total_rec_catch(dusky_rds$data, remove_non_standard = FALSE)


bull <- save_catch(this_species = "bull shark",
                   out_folder = here::here("data-raw/hms_mrip"),
                   catch_type = "landings")
...
sandbar <- save_catch(this_species = "sandbar shark",
                   out_folder = here::here("data-raw/hms_mrip"),
                   catch_type = "landings")
sandbar_rds <- readRDS(sandbar)
sandbar_rds
sandbar_mrip <- create_total_rec_catch(sandbar_rds$data, remove_non_standard = FALSE)


blacknose <- save_catch(this_species = "blacknose shark",
                   out_folder = here::here("data-raw/hms_mrip"),
                   catch_type = "landings")
blacknose_rds <- readRDS(blacknose)
blacknose_rds
blacknose_mrip <- create_total_rec_catch(blacknose_rds$data, remove_non_standard = FALSE)


silky <- save_catch(this_species = "silky shark",
                        out_folder = here::here("data-raw/hms_mrip"),
                        catch_type = "landings")
silky_rds <- readRDS(silky)
silky_rds
silky_mrip <- create_total_rec_catch(silky_rds$data, remove_non_standard = FALSE)


blacktip <- save_catch(this_species = "blacktip shark",
                        out_folder = here::here("data-raw/hms_mrip"),
                        catch_type = "landings")
blacktip_rds <- readRDS(blacktip)
blacktip_rds
blacktip_mrip <- create_total_rec_catch(blacktip_rds$data, remove_non_standard = FALSE)


whitetip <- save_catch(this_species = "oceanic whitetip shark",
                        out_folder = here::here("data-raw/hms_mrip"),
                        catch_type = "landings")
whitetip_rds <- readRDS(whitetip)
whitetip_rds
whitetip_mrip <- create_total_rec_catch(whitetip_rds$data, remove_non_standard = FALSE)


spinner <- save_catch(this_species = "spinner shark",
                        out_folder = here::here("data-raw/hms_mrip"),
                        catch_type = "landings")
spinner_rds <- readRDS(spinner)
spinner_rds
spinner_mrip <- create_total_rec_catch(spinner_rds$data, remove_non_standard = FALSE)


blue <- save_catch(this_species = "blue shark",
                        out_folder = here::here("data-raw/hms_mrip"),
                        catch_type = "landings")
...
lemon <- save_catch(this_species = "lemon shark",
                        out_folder = here::here("data-raw/hms_mrip"),
                        catch_type = "landings")
lemon_rds <- readRDS(lemon)
lemon_rds
lemon_mrip <- create_total_rec_catch(lemon_rds$data, remove_non_standard = FALSE)


finetooth <- save_catch(this_species = "finetooth shark",
                        out_folder = here::here("data-raw/hms_mrip"),
                        catch_type = "landings")
finetooth_rds <- readRDS(finetooth)
finetooth_rds
finetooth_mrip <- create_total_rec_catch(finetooth_rds$data, remove_non_standard = FALSE)
